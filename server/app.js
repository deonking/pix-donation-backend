...
const auth = `Bearer ${SECRET_KEY}`;

app.post('/create-pix', async (req, res) => {
    try {
        // Validação básica do payload
        const { amount, payer, description } = req.body;
        if (!amount || !payer || !payer.name || !payer.document) {
            return res.status(400).json({ message: 'Campos obrigatórios faltando: amount, payer.name, payer.document' });
        }
...
app.get('/transaction/:id', async (req, res) => {
    try {
        ...
        console.error('Erro ao verificar transação:', {
            message: error.message,
            status: error.response?.status,
            data: error.response?.data,
        });
